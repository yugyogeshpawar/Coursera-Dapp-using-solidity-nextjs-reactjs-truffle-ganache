...
function index({ Hero, className, handleOpen }) {
  return (
    <>
      <div
        className={`bg w-full lg:overflow-x-hidden whitespace-nowrap  ${className} ${Hero.bgColor} overflow-hidden`}
      >
        <div className="w-full flex px-2 md:px-10 max-w-7xl text-sm md:text-base m-auto items-center justify-between">
          <div className="flex py-4 items-center">
            ...
            <div
              className="font-bold sm:px-5 text-white whitespace-pre-wrap 
                            "
            >
              {Hero.Heading ? Hero.Heading : "Coursera 😈"}
            </div>
          </div>
          <button
            onClick={handleOpen}
            className={` ${Hero.buttonColor} font-bold p-2 lg:p-4 rounded text-sm my-1`}
          >
