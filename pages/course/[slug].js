...
import {
  CourseinProfessionalCertificates,
  CourseHero,
  WhatisProfessionalCertificate,
  TopBars,
  CourseTopBar,
  WhatYouWillLearn,
  SideBar,
  ...
  if (courseProps) {
    ...
    return (
      <div className="relative flex justify-center flex-col ">
        ...
        <div
          className={` z-[60] bg-white w-full top-0 ${offset2} ${offSetMob} items-center   `}
        >
          <CourseTopBar
            Hero={Hero}
        ...
        <div
          className={` z-[60]  bg-white w-full bottom-0 ${offSetMob} 
                items-center whitespace-nowrap overflow-hidden flex lg:hidden`}
        >
          <CourseTopBar
            Hero={Hero}
            handleOpen={() => setCoursePopTitle(Hero.Heading)}
          />{" "}
        </div>
        <div>
          <TopBars />
        ...
        <div className="w-full flex justify-center flex-col">
          ...
          <Instructor /> <div className="h-10 lg:h-28" id="faq"></div>{" "}
          <div>
            <FAQ />
          </div>
        </div>
        ...
        <div
          className="w-0 h-0 bg-gradient-to-r from-[#1F70C1] to-[#000000] text-black 
                last:bg-[#2B2B33] bg-[#FBBC05] lg:bg-black  "
        >
          <div className="h-0 w-0 bg-[#1C2B33] "></div>
        </div>
        <ModelView
          coursePopTitle={coursePopTitle}
          courseId={courseId}
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(`/api/blog?slug=${slug}.json`);
